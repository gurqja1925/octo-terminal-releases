...
2. [주요 기능](#주요-기능)
   ...
   - [노트 템플릿](#10--노트-템플릿)
   - [테마와 폰트](#11--테마와-폰트)
   - [키보드 단축키](#12-️-키보드-단축키)
   - [볼트 (비밀 저장소)](#13--볼트)
   - [히스토리](#14--히스토리)
   - [음성 입력 (Whisper)](#15--음성-입력-whisper)
   - [텔레그램 연동](#16--텔레그램-연동)
   - [스크린샷 / 이미지 붙여넣기](#17--스크린샷--이미지-붙여넣기)
   - [Octo Commands](#18--octo-commands)
   - [명령 팔레트](#19--명령-팔레트)
   - [CJK 입력 (한/중/일)](#20-️-cjk-입력)
   - [Octo Orchestra](#21--octo-orchestra)
3. [설치 방법](#설치-방법)
4. [사용 방법](#사용-방법)
5. [문제 해결](#문제-해결)
...
SSH를 통해 원격 서버의 프로젝트를 로컬처럼 작업할 수 있습니다.

**프로젝트 추가:**
1. "+" 버튼으로 새 프로젝트 생성
2. SSH 프로젝트 옵션 선택
3. 호스트, 포트, 사용자명, SSH 키 경로, 원격 디렉토리 입력
4. 연결 테스트 후 프로젝트 생성

**기능:**
- 원격 파일 트리 탐색 (폴더 열기/접기)
- 원격 파일 읽기/쓰기
- 터미널이 원격 서버에 자동 접속 (SSH + PTY 할당)
- 앱 재시작 시 SSH 세션 자동 복원

**주의사항:**
- SSH 키 인증을 권장합니다 (비밀번호 인증은 미지원)
- 원격 서버에 `sh` 쉘이 필요합니다
...
여러 AI 에이전트를 동시에 실행하여 하나의 프로젝트를 협업하는 멀티에이전트 오케스트레이션 시스템입니다.
...
- **비서 (Octopus)**: 팀을 모니터링하고, 진행 상황을 리뷰하고, 보고서를 작성합니다. 코드를 직접 수정하지 않습니다.
- **에이전트 (Squid)**: 각자 독립적인 Git 워크트리에서 할당된 작업을 수행하고, 완료 후 자동으로 메인 브랜치에 병합합니다.

**사전 준비:**

| 항목 | 설명 |
|------|------|
| Git 저장소 | 프로젝트가 Git으로 초기화되어 있어야 합니다 |
| 커밋 상태 | 미커밋 변경사항은 자동으로 stash됩니다 |
| AI CLI | Claude, Kimi, Codex 등 AI CLI가 설치되어 있어야 합니다 |

**사용 방법:**

1. **오케스트라 열기**
   - 터미널 탭바 하단의 지휘봉 아이콘 클릭, 또는
   - 상단 도구 바의 Orchestra 버튼 클릭

2. **비서 설정**
   - CLI 선택 (Claude 권장 — 폴링, 리뷰, 보고서 작성에 적합)
   - 비서은 에이전트들의 진행 상황을 모니터링하고 최종 리뷰를 담당

3. **에이전트 추가**
   - "+" 버튼으로 에이전트(Squid) 추가 (최대 6개)
   - 각 에이전트에 닉네임, CLI 종류, 작업 내용을 설정
   - 작업은 구체적으로 작성 (예: "로그인 API에 JWT 인증 추가")

4. **시작 (Go!)**
   - 시작 버튼 클릭 시 Preflight 검증 실행:
     - Git 저장소 확인
     - 미커밋 변경사항 자동 stash
     - 이전 세션 잔여 워크트리 정리
   - 검증 통과 후 각 에이전트별로 독립 Git 워크트리와 브랜치가 생성
   - 비서 + 에이전트 각각 별도 창에서 실행

5. **작업 진행**
   - 각 에이전트는 독립된 워크트리에서 작업 (서로 충돌 없음)
   - 비서은 30초마다 `git log` / `git diff`로 진행 상황 폴링
   - 사이드바에서 각 에이전트의 상태를 실시간 확인 (작업중/완료/오류)

6. **자동 병합**
   - 에이전트가 커밋 후 CLI를 종료하면 자동 감지 (5초 폴링)
   - 커밋이 있으면 → 자동으로 메인 브랜치에 `git merge` 실행
   - 충돌 발생 시 → `git merge --abort` 후 브랜치를 보존하여 비서에게 전달
   - 각 에이전트는 병합 완료 후 자동으로 창이 닫힘

7. **리뷰 단계**
   - 모든 에이전트 완료 시 자동으로 "리뷰" 단계 진입
   - 비서이 병합 결과를 확인하고, 미병합 브랜치가 있으면 직접 병합
   - `docs/orchestra-report.md`에 최종 보고서 작성
   - "확인 완료 — 전체 종료" 버튼으로 비서 포함 모든 창 종료

**작업 분배 팁:**
- 작업을 나누기 전에 소스를 기능별 파일로 분리하면 병합 충돌을 줄일 수 있습니다
- 같은 파일을 여러 에이전트가 수정하면 충돌 확률이 높아집니다
- 에이전트별 작업 범위를 명확하게 지정하세요 (파일명, 함수명 등)

**중지 옵션:**

| 버튼 | 동작 |
|------|------|
| 에이전트 중지 | 에이전트만 중지하고 커밋된 작업은 병합, 비서은 유지 |
| 전체 중지 | 비서 포함 모든 창 종료 |

**설정 저장:**
- 비서 CLI, 에이전트 수, 닉네임, 작업 내용이 프로젝트별로 자동 저장됩니다
- 다음 세션에서 이전 설정이 그대로 복원됩니다
- "초기화" 버튼으로 기본 상태로 되돌릴 수 있습니다

**프로젝트별 독립 실행:**
- 각 프로젝트마다 별도의 오케스트라를 실행할 수 있습니다
- 5개 프로젝트에서 5개의 오케스트라를 동시에 실행 가능
- 워크트리와 에이전트 창은 프로젝트별로 격리됩니다
...
> v0.1.6부터 Apple Developer ID로 코드 서명되어 있어 Gatekeeper 경고 없이 바로 실행됩니다.

만약 이전 버전(v0.1.5 이하)에서 "손상되었다"는 경고가 나타나면:
...
1. `.msi` 파일 더블클릭
2. 게시자(`Publisher`)가 신뢰 가능한 코드서명 주체인지 확인
3. SmartScreen 경고가 뜨면 "추가 정보" > 파일/게시자 확인 후 "실행"
4. 설치 마법사를 따라 진행
5. 완료 후 자동 실행
...
**서명/경고 대응 참고**
- 공식 릴리즈는 Windows 코드서명(sha256 + timestamp) 후 배포됩니다.
- `Unknown Publisher`가 보이면 최신 릴리즈 파일인지, 다운로드가 중간에 변조되지 않았는지 먼저 확인하세요.
- OV 인증서는 초기에 SmartScreen 경고가 남을 수 있고, 다운로드/실행 평판이 쌓이면 점진적으로 줄어듭니다.
- EV 인증서는 초기 SmartScreen 평판에 더 유리합니다.

코드서명 운영/자동화 가이드: `docs/인증서대응.md`
...
```
v0.1.6 이상은 Apple 코드 서명이 적용되어 이 문제가 발생하지 않습니다.
이전 버전(v0.1.5 이하)을 사용 중이라면:
xattr -cr "/Applications/Octo Terminal.app"
```
...
**Q: Windows 설치 때 Publisher/SmartScreen 경고가 떠요**
```
공식 릴리즈 최신 파일인지 먼저 확인
게시자(Publisher) 서명 정보 확인
배포/운영 가이드: docs/인증서대응.md
```
